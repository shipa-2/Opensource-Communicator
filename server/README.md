...
При прослушивании на wildcard-адресе `0.0.0.0` или `::` доменная часть логина
считается сетевым псевдонимом. Например, созданный пользователь
`shipa@localhost` может войти как `shipa@192.168.1.26`; после авторизации
сервер использует канонические логин и домен из базы. Если одинаковые
имя пользователя и пароль существуют в нескольких доменах, неоднозначный
псевдоним отклоняется.
