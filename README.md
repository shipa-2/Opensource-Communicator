...
- **OpenSource Communicator ↔ OSC** — `Openping!`, обмен аватаркой (`**fnm=avatar.png;…**`) и темой (обои + затемнение; `Themeapplied!` после применения); см. [PROTOCOL.md](PROTOCOL.md#расширения-opensource-communicator-im)
- **Обои** — фон главного окна, затемнение интерфейса и списков, «Поделиться темой» OSC-пиру
- **Интерфейс звонка**: крупный круглый аватар с подсветкой голоса, заметки, кнопки с текстовыми подписями
...
Между клиентами **OpenSource Communicator** (после `Openping!`) доступны обмен фото аватарки и **темой** (обои + затемнение интерфейса/списков). После применения темы получатель отправляет отправителю `Themeapplied!` — в чате появится «*Имя* применил вашу тему». Подробности: [PROTOCOL.md — расширения OSC](PROTOCOL.md#расширения-opensource-communicator-im).

### Контекстное меню

- **ПКМ по контакту**: Позвонить (с выбором номера), Сообщение, Заметка, Удалить, Экспортировать
...
При звонке отображается окно с:
- Имя/номер собеседника
- Круглый аватар с цветом контакта (подсвечивается при речи)
- Статус и таймер
- Заметки
