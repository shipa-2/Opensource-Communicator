...
- Контакты домена и presence; **личные контакты на сервере** — добавленные вручную или импортированные сохраняются в адресной книге ВАТС и доступны после входа с любого компьютера (не только в локальных настройках)
- Импорт/экспорт личных контактов CSV и vCard (импорт отправляет записи на сервер)
