...
// USB device uevent: PRODUCT=b0e/24db/113 (lowercase hex), Jabra vendor 0b0e.
...
bool JabraHeadset::ensureDevice()
{
    ...
    for (const QString &entry : entries) {
        ...
        if (::fcntl(fd, F_GETFL) != -1) {
            // Reading requires O_RDWR; without it only the LED writes work.
...
bool JabraHeadset::writeBits(unsigned char bits)
{
    ...
    if (!sendBits(m_fd, bits)) {
        // Device may have been replugged (BT dongle reconnection); rescan once.
        ::close(m_fd);
        m_fd = -1;
        if (ensureDevice()) {
...
void JabraHeadset::pollInput()
{
    ...
    while ((n = ::read(m_fd, buf, sizeof(buf))) > 0) {
        ...
        const bool hook = buf[1] & 0x01; // Telephony 0x20 Hook Switch
        if (hook && !m_hookPressed) {
            ...
            if (!m_longPressTimer) {
                ...
                connect(m_longPressTimer, &QTimer::timeout, this, [this]() {
                    m_hookPressed = false; // release after a long press is ignored
                    emit hookLongPress();
                });
            }
